    ...
    def _assert_invariants(self, solver, tasks, time_blocks):
            ...
            if not is_present:
                if task.chunks:
                    for c in task.chunks:
                        self.assertFalse(
                            solver.value(c["presence_var"]), f"Task absent but chunk present for {task.name}"
                        )
                        self.assertEqual(
                            solver.value(c["size_var"]), 0, f"Task absent but chunk has size > 0 for {task.name}"
                        )
                continue
        ...
        for i in range(len(strict_intervals) - 1):
            ...
            self.assertLessEqual(
                curr_end,
                next_start,
                f"Overlap: {curr_name} ({curr_start}-{curr_end}) intersects {next_name} ({next_start}-{next_end})",
            )
