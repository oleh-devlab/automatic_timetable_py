...
def create_model(user_tasks, time_blocks, max_horizon_days=14):
    ...
    for i, task in enumerate(user_tasks):
        needs_chunking = (
            ...
            and task.duration > task.min_chunk_duration
        )
        ...
        if needs_chunking:
            ...
            for c in range(max_chunks):
                ...
                ext_size = model.new_int_var(0, task.max_chunk_duration + task.break_duration, f'ext_size_{i}_chunk_{c}')
            ...
            for c, chunk in enumerate(task.chunks):
                ...
                if c > 0:
                    ...
                    model.add(
                        chunk['start_var'] >= prev_chunk['end_var'] + task.break_duration
                    ).only_enforce_if(chunk['presence_var'])
            ...
            model.add(
                sum(chunk['size_var'] for chunk in task.chunks) == task.duration
            ).only_enforce_if(task.presence_var)
